...
export function getRenderKeyword(text, keyword) {
  if (text && keyword) {
    ...
    return (
      <span className="keyword" dangerouslySetInnerHTML={{ __html: html }} />
    )
  }
}
...
export function getSecond(parts) {
  if (parts) {
    return (
      Number(parts[1] || 0) * 60 + Number(parts[2] || 0) + Number(parts[3] || 0)
    )
  }
  return 0
}
...
export function formatLyric(lines, timePattern) {
  ...
  lines.forEach((item, i) => {
    ...
    if (parts) {
      ...
      const nextParts =
        i !== lines.length - 1 && lines[i + 1].match(timePattern)
...
export function getLyric(lyricData) {
  if (lyricData && Object.keys(lyricData)) {
    ...
    const orderKeys = Object.keys(originLyric)
      .map(Number)
      .sort((a, b) => a - b)
    ...
    return artists.map((artist, index) => {
      ...
      return (
        <Fragment key={id}>
          <Link
            to={`/artist/${id}`}
            title={artists.map((v) => v.name).join(' / ')}
          >
            {name}
          </Link>
