...
export async function requestQrKey() {
  ...
  return request(
    `${API.qrKey.url}${stringify({ t: timestamp }, { addQueryPrefix: true })}`
  )
}
...
export async function requestFollows(params) {
  return request(
    `${API.follows.url}${stringify(params, { addQueryPrefix: true })}`
  )
}

export async function requestListeningRankingList(params) {
  return request(
    `${API.listeningRankingList.url}${stringify(params, { addQueryPrefix: true })}`
  )
}

export async function requestPlaylist(params) {
  return request(
    `${API.playlist.url}${stringify(params, { addQueryPrefix: true })}`
  )
}

export async function requestRadios(params) {
  return request(
    `${API.radios.url}${stringify(params, { addQueryPrefix: true })}`
  )
}
